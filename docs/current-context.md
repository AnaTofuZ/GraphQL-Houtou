...
このリポジトリの主系は、旧 `compiled_ir` / `XS::Execution` 系ではなく、
`Runtime` / `VM` ベースの新実装です。
...
- 旧 `compiled_ir` / `Execution::PP` / `XS::Execution` の active dependency は削除済み
...
- parser / graphql-js 互換でまだ必要な実体は
  ...
  に責務分離して保持
- 今後の高速化は old corridor widening ではなく、runtime/VM 本体で進める
- 特に注力するのは:
