  ...
  CODE:
    {
      ...
      for (i = 0; i < state->pending_count; i++) {
        av_push(ret, newSVpvn(state->pending_entries[i].result_name_pv, state->pending_entries[i].result_name_len));
      }
      RETVAL = newRV_noinc((SV *)ret);
    }
  OUTPUT:
  ...
  CODE:
    {
      ...
      IV i;
      for (i = 0; i < state->pending_count; i++) {
        if (state->pending_entries[i].payload_kind == GQL_VM_PENDING_OUTCOME_PTR) {
          gql_runtime_vm_outcome_t *outcome = state->pending_entries[i].payload.outcome_ptr;
          gql_runtime_vm_outcome_incref(outcome);
          av_push(ret, gql_runtime_vm_new_handle_sv(aTHX_ "GraphQL::Houtou::Runtime::Outcome", outcome));
        } else if (state->pending_entries[i].payload_kind == GQL_VM_PENDING_PROMISE_SV) {
          av_push(ret, newSVsv(state->pending_entries[i].payload.promise_sv));
        }
      }
      RETVAL = newRV_noinc((SV *)ret);
    }
...
SV *
block_frame_build_pending_aggregate_xs(frame, promise_code)
    SV *frame
    SV *promise_code
  CODE:
    {
      ...
      for (i = 0; i < state->pending_count; i++) {
        if (state->pending_entries[i].payload_kind == GQL_VM_PENDING_OUTCOME_PTR) {
          ...
          av_push(pending_av, newSVsv(state->pending_entries[i].payload.promise_sv));
        }
      }
