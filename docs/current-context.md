...
`NativeRuntime->execute_compact_program(...)` に委譲した fresh `./Build build` 済み環境で、
...
- current runtime mainline
  - `nested_variable_object`
    - `houtou_runtime_cached_perl` median `18562/s`
    - `houtou_runtime_native_bundle` median `588339/s`
  - `list_of_objects`
    - `houtou_runtime_cached_perl` median `14231/s`
    - `houtou_runtime_native_bundle` median `502626/s`
  - `abstract_with_fragment`
    - `houtou_runtime_cached_perl` median `16641/s`
    - `houtou_runtime_native_bundle` median `556709/s`

- old compiled-ir mainline reference
  - commit `43c06d9` の `docs/current-context.md`
  - `nested_variable_object`
...
- current native bundle は旧 `compiled_ir` 比で大幅に速い
  - `nested_variable_object`: 約 `7.3x`
  - `list_of_objects`: 約 `8.4x`
  - `abstract_with_fragment`: 約 `13.3x`
