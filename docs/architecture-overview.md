...
P0 仕様適合(リクエスト validation / エラー envelope / 結果コアーション /
Non-Null 伝播)まで入った現在の全体像。設計原則は `architecture.md`、
非同期・DataLoader の product 方針は `product-roadmap.md`、個々のモジュールの
責務は `module-map.md` を参照。この文書は**データの流れと実行時の構造**を
図で押さえるための地図である。
...
原則(architecture.md の要約): **キャッシュするのは実行計画**であり、
...
単位のバッチング。レベル順 BFS の上位互換 — bfs-execution-analysis.md)。
