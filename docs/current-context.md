...
`Runtime::NativeBundle` を外した fresh `./Build build` 済み環境で、
...
- current runtime mainline
  - `nested_variable_object`
    - `houtou_runtime_cached_perl` median `18038/s`
    - `houtou_runtime_native_bundle` median `570071/s`
  - `list_of_objects`
    - `houtou_runtime_cached_perl` median `13829/s`
    - `houtou_runtime_native_bundle` median `496824/s`
  - `abstract_with_fragment`
    - `houtou_runtime_cached_perl` median `16330/s`
    - `houtou_runtime_native_bundle` median `539309/s`
...
- current native bundle は旧 `compiled_ir` 比で大幅に速い
  - `nested_variable_object`: 約 `7.0x`
  - `list_of_objects`: 約 `8.3x`
  - `abstract_with_fragment`: 約 `12.9x`
