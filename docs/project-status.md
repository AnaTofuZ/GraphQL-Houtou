...
- `GraphQL::Houtou::GraphQLJS::Canonical` は executable document について
  `graphqljs_build_executable_document_xs()` を優先利用する。
- object value を含む executable も XS builder 側で通るようにしたため、
  executable path では `GraphQLPerlToGraphQLJS` への fallback は不要になった。
- `GraphQLPerlToGraphQLJS` は SDL と非 executable 定義、ならびに残る graphql-js 差分のために
  まだ必要だが、削減対象は executable value 変換から SDL / extension 系へ移った。
