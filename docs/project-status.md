...
- object value を含む executable も XS builder 側で通るようにしたため、
  executable path では Perl fallback converter への fallback は不要になった。
...
- 2026-04-03 時点では `graphql-js` parser 自体を XS 専用経路とし、
  legacy AST から graphql-js AST への Perl fallback conversion は廃止した。
