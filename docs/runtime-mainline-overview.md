...
モジュールごとの役割を一覧で確認したい場合は `docs/module-map.md` を参照してください。
...
- 旧テスト資産は削除済みで、必要なら Git 履歴(`legacy-tests/`)を参照します。
...
- 旧 parser / validation / execution の回帰資産は削除済みです(有効な
  ケースは active suite に移植済み。原本は Git 履歴の `legacy-tests/`)。
