...
### R5. valgrind / LeakSanitizer の一回転 — 実施済み(残 1 件追跡中)
...
3. **fast_json/execute 経路の path frame leak(未修正・追跡中)**。
   ...
   CI の valgrind ゲートは検証済み leak-clean サブセット(t/44 /
   t/49 / t/50 / t/47)で有効化。2・3 の修正後に t/35 / t/36 / t/40 /
   t/48 を含む全テストへ広げる。**いずれも per-request unbounded では
   ない**(soak +416KB/20k が bounded の証左)ため DoS ではないが、
   開発者判断でリリース前修正が望ましい追跡項目。
