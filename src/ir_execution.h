...
static int gql_ir_native_field_complete_abstract_family_fallback_result(
  pTHX_ gql_ir_vm_exec_state_t *state,
...
gql_ir_vm_exec_state_complete_current_field(
  ...
  switch (op) {
    ...
    case GQL_IR_NATIVE_FIELD_OP_COMPLETE_ABSTRACT_TAG:
      return gql_ir_native_field_complete_abstract_family_fallback_result(
        aTHX_ state,
        GQL_IR_ABSTRACT_DISPATCH_TAG
      );
    case GQL_IR_NATIVE_FIELD_OP_COMPLETE_ABSTRACT_RESOLVE_TYPE:
      return gql_ir_native_field_complete_abstract_family_fallback_result(
        aTHX_ state,
        GQL_IR_ABSTRACT_DISPATCH_RESOLVE_TYPE
      );
    case GQL_IR_NATIVE_FIELD_OP_COMPLETE_ABSTRACT_POSSIBLE_TYPES:
      return gql_ir_native_field_complete_abstract_family_fallback_result(
        aTHX_ state,
        GQL_IR_ABSTRACT_DISPATCH_POSSIBLE_TYPES
      );
...
gql_ir_try_complete_sync_list_into_outcome(
  ...
  for (i = 0; i <= result_len; i++) {
    ...
    if (!item_data_sv && !item_completed_sv) {
...
gql_ir_native_field_complete_generic_fallback_result(
  ...
  if (gql_execution_complete_field_value_catching_error_xs_lazy_sync_native_outcome(
    ...
    gql_ir_sync_outcome_move_to_frame(&outcome, frame);
    return frame->outcome_kind != GQL_IR_NATIVE_FIELD_OUTCOME_NONE;
  }
  return 0;
}

static int
gql_ir_native_field_complete_object_family_fallback_result(
  pTHX_ gql_ir_vm_exec_state_t *state
) {
  gql_ir_native_exec_env_t *env = state ? state->env : NULL;
  gql_ir_vm_exec_cursor_t *cursor = state ? &state->cursor : NULL;
  ...
  if (!env || !cursor || !field_def_sv || !nodes_sv || !return_type_sv || !lazy_info || !frame) {
    return 0;
  }
...
static int
gql_ir_native_field_complete_list_family_fallback_result(
  pTHX_ gql_ir_vm_exec_state_t *state
) {
  gql_ir_native_exec_env_t *env = state ? state->env : NULL;
  gql_ir_vm_exec_cursor_t *cursor = state ? &state->cursor : NULL;
...
static int
gql_ir_native_field_complete_abstract_family_fallback_result(
  pTHX_ gql_ir_vm_exec_state_t *state,
  gql_ir_abstract_dispatch_kind_t abstract_dispatch_kind
) {
  gql_ir_native_exec_env_t *env = state ? state->env : NULL;
  ...
  gql_execution_sync_outcome_reset(&outcome);
  if (!gql_execution_complete_abstract_field_value_catching_error_xs_lazy_sync_native_outcome_with_table_and_dispatch(
        aTHX_
        gql_ir_native_env_context(env),
        gql_ir_native_env_parent_type(env),
        field_def_sv,
...
gql_ir_native_field_complete_object_result(
  ...
  return gql_ir_native_field_complete_object_family_fallback_result(aTHX_ state);
}
...
gql_ir_native_field_complete_list_result(
  ...
  return gql_ir_native_field_complete_list_family_fallback_result(aTHX_ state);
}
...
gql_ir_init_native_field_ops(gql_ir_compiled_root_field_plan_entry_t *entry) {
  ...
  if (meta->completion_dispatch_kind == GQL_IR_NATIVE_COMPLETION_TRIVIAL) {
    ...
    switch (meta->abstract_dispatch_kind) {
      case GQL_IR_ABSTRACT_DISPATCH_TAG:
        meta->ops[op_count++] = GQL_IR_NATIVE_FIELD_OP_COMPLETE_ABSTRACT_TAG;
        break;
      case GQL_IR_ABSTRACT_DISPATCH_RESOLVE_TYPE:
        meta->ops[op_count++] = GQL_IR_NATIVE_FIELD_OP_COMPLETE_ABSTRACT_RESOLVE_TYPE;
        break;
      case GQL_IR_ABSTRACT_DISPATCH_POSSIBLE_TYPES:
        meta->ops[op_count++] = GQL_IR_NATIVE_FIELD_OP_COMPLETE_ABSTRACT_POSSIBLE_TYPES;
        break;
      default:
...
dispatch:
  ...
  switch (op) {
    ...
    case GQL_IR_NATIVE_FIELD_OP_COMPLETE_ABSTRACT_TAG: goto op_complete_abstract_tag;
    case GQL_IR_NATIVE_FIELD_OP_COMPLETE_ABSTRACT_RESOLVE_TYPE: goto op_complete_abstract_resolve_type;
    case GQL_IR_NATIVE_FIELD_OP_COMPLETE_ABSTRACT_POSSIBLE_TYPES: goto op_complete_abstract_possible_types;
...
op_complete_abstract_tag:
  if (!gql_ir_vm_exec_state_complete_current_field(
        aTHX_
        state,
        GQL_IR_NATIVE_FIELD_OP_COMPLETE_ABSTRACT_TAG
      )) {
    goto op_fail;
  }
  if (frame->outcome_kind == GQL_IR_NATIVE_FIELD_OUTCOME_NONE) {
...
op_complete_abstract_resolve_type:
  if (!gql_ir_vm_exec_state_complete_current_field(
        aTHX_
        state,
        GQL_IR_NATIVE_FIELD_OP_COMPLETE_ABSTRACT_RESOLVE_TYPE
      )) {
    goto op_fail;
  }
  if (frame->outcome_kind == GQL_IR_NATIVE_FIELD_OUTCOME_NONE) {
...
op_complete_abstract_possible_types:
  if (!gql_ir_vm_exec_state_complete_current_field(
        aTHX_
        state,
        GQL_IR_NATIVE_FIELD_OP_COMPLETE_ABSTRACT_POSSIBLE_TYPES
      )) {
    goto op_fail;
  }
  if (frame->outcome_kind == GQL_IR_NATIVE_FIELD_OUTCOME_NONE) {
...
gql_ir_vm_field_meta_from_seed(
  ...
  meta->abstract_dispatch_kind = seed->abstract_dispatch_kind;
  entry->meta = meta;
...
gql_ir_compiled_root_field_plan_from_sv(pTHX_ SV *root_field_plan_sv) {
  ...
  for (field_i = 0; field_i < field_count; field_i++) {
    ...
    gql_ir_abstract_dispatch_kind_t abstract_dispatch_kind = GQL_IR_ABSTRACT_DISPATCH_NONE;
    SV **nodes_svp;
    AV *nodes_av;
    SV **first_node_svp;
    SV **node_count_svp;
    ...
    if (completion_dispatch_kind == GQL_IR_NATIVE_COMPLETION_GENERIC
      ...
      if (SvROK(*type_svp)
        ...
        if (SvROK(*type_svp) && SvTYPE(SvRV(*type_svp)) == SVt_PVHV) {
          HV *abstract_hv = (HV *)SvRV(*type_svp);
          SV **tag_resolver_svp = hv_fetch(abstract_hv, "tag_resolver", 12, 0);
          SV **resolve_type_svp = hv_fetch(abstract_hv, "resolve_type", 12, 0);

          if (tag_resolver_svp && SvOK(*tag_resolver_svp)) {
            abstract_dispatch_kind = GQL_IR_ABSTRACT_DISPATCH_TAG;
          } else if (resolve_type_svp && SvOK(*resolve_type_svp)) {
            abstract_dispatch_kind = GQL_IR_ABSTRACT_DISPATCH_RESOLVE_TYPE;
          } else {
            abstract_dispatch_kind = GQL_IR_ABSTRACT_DISPATCH_POSSIBLE_TYPES;
          }
        } else {
          abstract_dispatch_kind = GQL_IR_ABSTRACT_DISPATCH_POSSIBLE_TYPES;
        }
    ...
    meta_seed.abstract_dispatch_kind = abstract_dispatch_kind;
