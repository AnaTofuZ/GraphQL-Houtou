...
`runtime + program` の Perl descriptor hash を組み立てず part-based native load を使う
...
- current runtime mainline
  - `nested_variable_object`
    - `houtou_runtime_cached_perl` median `17998/s`
    - `houtou_runtime_native_bundle` median `566900/s`
  - `list_of_objects`
    - `houtou_runtime_cached_perl` median `13826/s`
    - `houtou_runtime_native_bundle` median `475664/s`
  - `abstract_with_fragment`
    - `houtou_runtime_cached_perl` median `16283/s`
    - `houtou_runtime_native_bundle` median `538751/s`
...
- current native bundle は旧 `compiled_ir` 比で大幅に速い
  - `nested_variable_object`: 約 `10.0x`
  - `list_of_objects`: 約 `8.2x`
  - `abstract_with_fragment`: 約 `13.9x`
