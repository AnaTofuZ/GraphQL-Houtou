...
`Runtime::Program` 削除後の fresh `./Build build` 済み環境で、
...
- current runtime mainline
  - `nested_variable_object`
    ...
    - `houtou_runtime_native_bundle` median `515525/s`
  - `list_of_objects`
    - `houtou_runtime_cached_perl` median `13916/s`
    - `houtou_runtime_native_bundle` median `452580/s`
  - `abstract_with_fragment`
    - `houtou_runtime_cached_perl` median `16486/s`
...
- current native bundle は旧 `compiled_ir` 比で大幅に速い
  - `nested_variable_object`: 約 `6.4x`
  - `list_of_objects`: 約 `7.5x`
  - `abstract_with_fragment`: 約 `11.7x`
