...
gql_execution_sync_outcome_export(
  ...
  switch (outcome->kind) {
    ...
    case GQL_EXECUTION_SYNC_OUTCOME_DIRECT_OBJECT_HV:
      if (data_out) {
        *data_out = outcome->data_sv ? newRV_noinc(outcome->data_sv) : newSV(0);
      }
      if (errors_out) {
        *errors_out = outcome->errors_av;
      }
...
gql_execution_complete_list_field_value_catching_error_xs_lazy_sync_native_outcome_with_items(
  ...
  for (i = 0; i <= result_len; i++) {
    ...
    if (exact_item_native_field_plan) {
      ...
      item_outcome.data_sv = (SV *)child_outcome.data_hv;
...
gql_execution_try_complete_abstract_runtime_type_or_name_sync_native_outcome(
  ...
  return gql_execution_complete_abstract_runtime_object_catching_error_xs_lazy_sync_native_outcome(
    aTHX_
    context,
    parent_type,
    field_def,
...
gql_execution_complete_abstract_runtime_object_catching_error_xs_lazy_sync_native_outcome(
  ...
  return gql_execution_complete_abstract_runtime_object_catching_error_xs_lazy_sync_native_outcome_impl(
    aTHX_
    context,
    parent_type,
    field_def,
