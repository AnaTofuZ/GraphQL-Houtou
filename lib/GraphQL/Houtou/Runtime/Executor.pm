...
sub _resolve_field_value {
  ...
  if ($resolver) {
    my $type = $state->runtime_schema->runtime_cache->{name2type}{ $instruction->return_type_name };
    return $resolver->($source, $state->context, undef, $type);
  }
...
sub _complete_object {
  ...
  return GraphQL::Houtou::Runtime::Outcome->new(kind => 'VALUE', value => undef)
    if !defined $value;
  ...
  return GraphQL::Houtou::Runtime::Outcome->new(
    kind => 'OBJECT',
    value => _execute_block($state, $child, $value),
  );
}
...
sub _complete_list {
  ...
  return GraphQL::Houtou::Runtime::Outcome->new(kind => 'VALUE', value => \@items);
}
